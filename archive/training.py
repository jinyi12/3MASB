...
def train_ccvep_bridge(
    ...
    ccvep_lr: float = 1e-4,
    lambda_CE: float = 1.0,
    # Early stopping for C-CVEP training
    ccvep_early_stopping: bool = True,
    ccvep_early_stopping_patience: int = 2000,
    ccvep_early_stopping_min_delta: float = 1e-4,
    # Common parameters
    batch_size: int = 64,
    grad_clip_norm: float = 1.0,
    verbose: bool = True,
    # CRITICAL FIX: Add raw data parameter
    marginal_data_raw: Dict[float, Tensor] = None
) -> List[Dict[str, float]]:
    ...
    try:
        ...
        # CRITICAL FIX: Extract raw couplings for RBF metric training
        if marginal_data_raw is not None and use_rbf_metric:
            _, raw_couplings = extract_couplings(marginal_data_raw)
        else:
            raw_couplings = couplings
    ...
    if use_rbf_metric and metric_handler is not None:
        print("\n--- Stage 0: RBF Metric Initialization and Pre-training ---")
        print("IMPORTANT: Training RBF metric on ORIGINAL (unnormalized) data for proper spatial structure")
        
        # CRITICAL FIX: Use raw (unnormalized) data for metric learning
        all_raw_data = torch.cat(raw_couplings, dim=0)

        # Initialize (K-means and bandwidths) on raw data
        ...
        # Pre-train weights (Stage 0b) on raw data
        ...
        print("Stage 0 completed. RBF metric trained on original data space.")
    else:
    ...
    # Early stopping variables for CCVEP
    best_ccvep_loss = float('inf')
    best_ccvep_epoch = -1
    best_ccvep_state = None
    epochs_no_improve_ccvep = 0
    ...
    for epoch in pbar_ccvep:
        ...
        # Early stopping check for CCVEP
        total_loss = loss_dict.get('loss_total', float('inf'))
        if ccvep_early_stopping and total_loss != float('inf'):
            if best_ccvep_loss - total_loss > ccvep_early_stopping_min_delta:
                best_ccvep_loss = total_loss
                best_ccvep_epoch = epoch
                # Save best CCVEP model state (v_theta + U_phi)
                best_ccvep_state = {
                    'v_theta': {k: v.cpu().clone() for k, v in bridge.v_theta.state_dict().items()},
                    'U_phi': {k: v.cpu().clone() for k, v in bridge.U_phi.state_dict().items()}
                }
                epochs_no_improve_ccvep = 0
            else:
                epochs_no_improve_ccvep += 1

            if epochs_no_improve_ccvep >= ccvep_early_stopping_patience:
                print(f"\nEarly stopping triggered at epoch {epoch} (best epoch {best_ccvep_epoch}, best loss {best_ccvep_loss:.6f}). Restoring best CCVEP weights.")
                if best_ccvep_state is not None:
                    bridge.v_theta.load_state_dict(best_ccvep_state['v_theta'])
                    bridge.U_phi.load_state_dict(best_ccvep_state['U_phi'])
                break
    ...
    # If training finished without early stopping but best_ccvep_state exists, restore best state
    if best_ccvep_state is not None:
        bridge.v_theta.load_state_dict(best_ccvep_state['v_theta'])
        bridge.U_phi.load_state_dict(best_ccvep_state['U_phi'])
        print(f"Restored best CCVEP model from epoch {best_ccvep_epoch} with loss {best_ccvep_loss:.6f}")
    
    print("Stage 2 completed. Using best CCVEP model for downstream analysis.")
    return loss_history