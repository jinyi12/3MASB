...
Examples:
    ...
    # Training on original data scale (no normalization)
    python train_ccvep_2stage.py --grf --use_unet --use_rbf_metric \
        --no_normalize --experiment_name "non_normalized_test"
...
# Import the normalization utility from the GLOW training script (KISS: reuse existing code)
from train_glow_flow import normalize_multimarginal_data
...
def setup_experiment(args) -> dict:
    ...
    config = {
        ...
        "rbf_epochs": getattr(args, 'rbf_epochs', 1000),
        "rbf_lr": getattr(args, 'rbf_lr', 1e-3),
        "mfm_epochs": getattr(args, 'mfm_epochs', 1000),
        "mfm_lr": getattr(args, 'mfm_lr', 1e-4),
        "ccvep_epochs": getattr(args, 'ccvep_epochs', 10000),
        "ccvep_lr": getattr(args, 'ccvep_lr', 1e-3),
        "lambda_CE": getattr(args, 'lambda_CE', 1.0),
        
        # Early stopping parameters (enabled by default, disabled with --disable_early_stopping)
        "early_stopping": not getattr(args, 'disable_early_stopping', False),
        "early_stopping_patience": getattr(args, 'early_stopping_patience', 1000),
        "early_stopping_min_delta": getattr(args, 'early_stopping_min_delta', 1e-3),
        "ccvep_early_stopping": not getattr(args, 'disable_early_stopping', False),
        "ccvep_early_stopping_patience": getattr(args, 'ccvep_early_stopping_patience', 2000),
        "ccvep_early_stopping_min_delta": getattr(args, 'ccvep_early_stopping_min_delta', 1e-4),

        # RBF metric parameters
        "rbf_clusters": getattr(args, 'rbf_clusters', 64),
        "rbf_epsilon": getattr(args, 'rbf_epsilon', 1e-3),
        "rbf_alpha": getattr(args, 'rbf_alpha', 1.0),
        "rbf_kappa": getattr(args, 'rbf_kappa', 1.0),
...
def generate_data(config):
    ...
    if config['data_type'] == 'grf':
        ...
        marginal_data_raw, data_dim = generate_multiscale_grf_data(
        ...
        marginal_data_raw, data_dim = generate_spiral_distributional_data(
    ...
    # CRITICAL FIX: Store raw data for RBF metric training
    config['marginal_data_raw'] = {t: samples.to(config['device']) for t, samples in marginal_data_raw.items()}
    
    # Conditionally normalize the data based on config
    if config.get('no_normalize', True):
        print("Skipping data normalization - training on original data scale")
        marginal_data = {t: samples.to(config['device']) for t, samples in marginal_data_raw.items()}
        # Set identity normalization parameters (mean=0, std=1)
        data_mean = torch.zeros(data_dim, device=config['device'])
        data_std = torch.ones(data_dim, device=config['device'])
        ...
        print("IMPORTANT: Training directly on original data space (no normalization)")
    else:
        # Normalize the data and store the mean/std (following GLOW Flow pattern)
        print("Normalizing multi-marginal data...")
        ...
        print("IMPORTANT: Raw data stored for RBF metric training on original space")
...
def create_model(config, data_dim):
    ...
    if config['use_rbf_metric']:
        ...
        # CRITICAL FIX: Pass normalization parameters to RBF metric handler
        data_mean = config.get('data_mean', torch.zeros(data_dim, device=config['device']))
        data_std = config.get('data_std', torch.ones(data_dim, device=config['device']))
        
        metric_handler = RBFMetricHandler(
            data_dim=data_dim, num_clusters=config['rbf_clusters'], epsilon=config['rbf_epsilon'],
            alpha=config['rbf_alpha'], device=config['device'], data_mean=data_mean, data_std=data_std
            , anisotropic=False
        ).to(config['device'])
    ...
    bridge = CCVEPBridge(
        ...
        data_mean=data_mean.to(config['device']), data_std=data_std.to(config['device'])
    ).to(config['device'])
...
def train_model(bridge, marginal_data, config):
    ...
    if config.get('no_normalize', False):
        print("IMPORTANT: Training directly on original data space (no normalization)")
        print("Both model training and RBF metric will work on original scale")
    elif config['use_rbf_metric']:
        print("IMPORTANT: RBF metric will be trained on original (unnormalized) data")
        print("This ensures proper spatial structure learning for geodesic computation")
    
    rbf_params = {
        'epochs': config['rbf_epochs'], 'lr': config['rbf_lr'], 'clusters': config['rbf_clusters'],
        'epsilon': config['rbf_epsilon'], 'alpha': config['rbf_alpha'], 'kappa': config['rbf_kappa'],
    }
    loss_history = train_ccvep_bridge(
        ...
        # Early stopping for MFM stage
        early_stopping=config.get('early_stopping', True),
        early_stopping_patience=config.get('early_stopping_patience', 1000),
        early_stopping_min_delta=config.get('early_stopping_min_delta', 1e-3),
        # CCVEP stage parameters
        ccvep_epochs=config['ccvep_epochs'], ccvep_lr=config['ccvep_lr'],
        lambda_CE=config['lambda_CE'],
        # Early stopping for CCVEP stage
        ccvep_early_stopping=config.get('ccvep_early_stopping', True),
        ccvep_early_stopping_patience=config.get('ccvep_early_stopping_patience', 2000),
        ccvep_early_stopping_min_delta=config.get('ccvep_early_stopping_min_delta', 1e-4),
        # Common parameters
        batch_size=config['batch_size'],
        grad_clip_norm=config['grad_clip_norm'], verbose=True,
        # CRITICAL FIX: Pass raw data for RBF metric training
        marginal_data_raw=config.get('marginal_data_raw', None)
    )
    if loss_history:
...
def validate_model(bridge, marginal_data, config):
    ...
    try:
        if config['data_type'] == 'grf':
            ...
            if config.get('no_normalize', False):
                # Data is already in original scale, no need to denormalize
                print("Using data directly (no normalization was applied)...")
                validation_generated_samples = {}
                validation_marginal_data = {}
                for t, samples in generated_samples.items():
                    samples_on_device = samples.to(config['device']) if samples.device != torch.device(config['device']) else samples
                    validation_generated_samples[t] = samples_on_device
                for t, samples in marginal_data.items():
                    samples_on_device = samples.to(config['device']) if samples.device != torch.device(config['device']) else samples
                    validation_marginal_data[t] = samples_on_device
            else:
                # CRITICAL: Denormalize both generated and ground truth data before metric calculation
                print("Denormalizing data for metric calculation...")
                validation_generated_samples = {}
                for t, samples in generated_samples.items():
                    # Ensure samples are on the same device as the bridge
                    samples_on_device = samples.to(config['device']) if samples.device != torch.device(config['device']) else samples
                    validation_generated_samples[t] = bridge.denormalize(samples_on_device)
                
                # The original marginal_data used for training is already normalized.
                # We need to denormalize it to get the original ground truth scale.
                validation_marginal_data = {}
                for t, samples in marginal_data.items():
                    # Ensure samples are on the same device as the bridge
                    samples_on_device = samples.to(config['device']) if samples.device != torch.device(config['device']) else samples
                    validation_marginal_data[t] = bridge.denormalize(samples_on_device)
            
            metrics = calculate_validation_metrics(validation_marginal_data, validation_generated_samples)
...
def visualize_results(bridge, marginal_data, config):
    ...
    print("\n--- Visualization ---")
    try:
        # Pass data to visualization (normalized or not based on config)
        # The visualization utility will handle normalization/denormalization appropriately
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Asymmetric Multi-Marginal Bridge with C-CVEP (Experiment-Ready)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python train_ccvep_2stage.py --grf --use_unet --use_rbf_metric
    python train_ccvep_2stage.py --grf --rbf_epochs 200 --mfm_epochs 300 --ccvep_epochs 500
    python train_ccvep_2stage.py --grf --experiment_name "test_01" --save_metrics
        """
    )
    
    # Core settings
    parser.add_argument('--grf', action='store_true', help='Use GRF data (recommended for C-CVEP)')
    parser.add_argument('--spiral', action='store_true', help='Use spiral data')
    parser.add_argument('--output_dir', type=str, help='Output directory for artifacts (overrides default)')
    parser.add_argument('--experiment_name', type=str, help='Optional experiment identifier used to name saved model files')
    
    # Architecture options
    parser.add_argument('--use_unet', action='store_true', help='Use U-Net architecture for spatial GRF data')
    parser.add_argument('--use_rbf_metric', action='store_true', help='Use learned RBF metric (recommended)')
    parser.add_argument('--hidden_dims', type=int, nargs='+', help='Hidden layer dimensions (e.g., 256 256 256)')
    parser.add_argument('--interpolator_hidden_dims', type=int, nargs='+', help='Interpolator hidden dimensions')
    parser.add_argument('--time_embedding_dim', type=int, help='Time embedding dimension')
    
    # Training stage parameters
    parser.add_argument('--rbf_epochs', type=int, help='RBF metric training epochs', default=1000)
    parser.add_argument('--rbf_lr', type=float, help='RBF metric learning rate', default=1e-3)
    parser.add_argument('--mfm_epochs', type=int, help='MFM interpolator pre-training epochs', default=1000)
    parser.add_argument('--mfm_lr', type=float, help='MFM interpolator learning rate', default=1e-3)
    parser.add_argument('--ccvep_epochs', type=int, help='C-CVEP main training epochs', default=10000)
    parser.add_argument('--ccvep_lr', type=float, help='C-CVEP main learning rate', default=1e-3)
    parser.add_argument('--lambda_CE', type=float, help='C-CVEP loss weight', default=1.0)
    # RBF metric parameters
    parser.add_argument('--rbf_clusters', type=int, help='Number of RBF clusters', default=64)
    parser.add_argument('--rbf_epsilon', type=float, help='RBF epsilon parameter', default=1e-3)
    parser.add_argument('--rbf_kappa', type=float, help='RBF kappa parameter', default=1.0)
    parser.add_argument('--rbf_alpha', type=float, help='RBF alpha parameter (metric sharpness)', default=1.0)

    # Training hyperparameters
    parser.add_argument('--batch_size', type=int, help='Training batch size', default=64)
    parser.add_argument('--grad_clip_norm', type=float, help='Gradient clipping norm', default=1.0)
    
    # Early stopping parameters
    parser.add_argument('--early_stopping_patience', type=int, help='MFM early stopping patience', default=1000)
    parser.add_argument('--early_stopping_min_delta', type=float, help='MFM early stopping minimum improvement', default=1e-3)
    parser.add_argument('--ccvep_early_stopping_patience', type=int, help='CCVEP early stopping patience', default=2000)
    parser.add_argument('--ccvep_early_stopping_min_delta', type=float, help='CCVEP early stopping minimum improvement', default=1e-4)
    parser.add_argument('--disable_early_stopping', action='store_true', help='Disable early stopping for all stages')

    # Data generation parameters
    parser.add_argument('--n_samples', type=int, help='Number of ground truth samples to generate', default=1024)
    parser.add_argument('--n_constraints', type=int, help='Number of time constraints/marginals', default=5)
    parser.add_argument('--resolution', type=int, help='Spatial resolution for GRF data', default=16)
    parser.add_argument('--micro_corr_length', type=float, help='GRF correlation length', default=0.1)
    parser.add_argument('--covariance_type', type=str, help='GRF kernel type (exponential or gaussian)', default="gaussian")
    parser.add_argument('--l_domain', type=float, help='GRF domain length', default=1.0)
    parser.add_argument('--h_max_factor', type=float, help='GRF H_max factor', default=0.5)
    parser.add_argument('--mean_val', type=float, help='GRF mean value', default=10.0)
    parser.add_argument('--std_val', type=float, help='GRF standard deviation', default=2.0)
    parser.add_argument('--n_samples_per_marginal', type=int, help='Samples per marginal for spiral data', default=256)
    parser.add_argument('--noise_std', type=float, help='Noise standard deviation for spiral data', default=0.1)

    # Bridge dynamics parameters
    parser.add_argument('--T', type=float, help='Time horizon', default=1.0)
    parser.add_argument('--sigma_reverse', type=float, help='Reverse SDE noise level', default=0.5)

    # Evaluation and visualization parameters
    parser.add_argument('--n_sde_steps', type=int, help='Number of SDE integration steps', default=100)
    parser.add_argument('--n_viz_particles', type=int, help='Number of particles for visualization (visuals only)', default=256)
    parser.add_argument('--n_validation_trajectories', type=int, help='Number of trajectories for validation metrics', default=1024)
    parser.add_argument('--save_metrics', action='store_true',
                      help='Save quantitative validation metrics (W2, MSE_ACF, Rel-F Frobenius) to output_dir/validation_metrics.json')
    parser.add_argument('--enable_covariance_analysis', action='store_true', 
                      help='Enable detailed covariance visualizations (may be heavy for large D)')
    parser.add_argument('--no_normalize', action='store_true',
                      help='Disable data normalization - train and evaluate on original data scale')
